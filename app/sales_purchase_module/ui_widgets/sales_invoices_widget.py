"""
Labeeb ERP - Sales Invoices Widget
واجهة فواتير المبيعات الكاملة
"""
...
class SalesInvoicesWidget(TranslatableWidget):
    """واجهة فواتير المبيعات الكاملة"""
    ...
    def on_item_selected(self):
        ...
        if item_id:
            try:
                ...
                if item:
                    self.price_input.setValue(float(item.sale_price))
    ...
    def save_document(self):
        ...
        try:
            # Validate
            if not self.customer_combo.currentData() and self.customer_combo.currentIndex() != 0:
            ...
            if payment_dialog.exec() == QDialog.Accepted:
                ...
                # Save invoice
                if self.backend.create_sales_invoice(invoice_data):
    ...
    def refresh_data(self):
        ...
        try:
            ...
            self.all_invoices = self.backend.get_all_sales_invoices()
    ...
    def filter_invoices(self):
        ...
        for row in range(self.invoices_table.rowCount()):
            ...
            if search_text:
                ...
                customer = self.invoices_table.item(row, 3).text().lower()
    ...
    def load_invoice(self, invoice_id):
        ...
        try:
            invoice = self.backend.get_sales_invoice_by_id(invoice_id)
            if not invoice:
                return
            
            self.current_invoice_id = invoice_id
            self.invoice_no_input.setText(invoice['invoice_no'])
            self.invoice_date_input.setDate(QDate.fromString(invoice['invoice_date'], "yyyy-MM-dd"))
            ...
            for item in invoice['items']:
                ...
                self.items_table.setItem(row, 4, QTableWidgetItem(f"{item['tax']:.2f}"))
                self.items_table.setItem(row, 5, QTableWidgetItem(f"{item['total']:.2f}"))
                
                self.invoice_items.append(item)
            
            self.discount_input.setValue(invoice['discount'])
            self.charges_input.setValue(invoice['charges'])
            self.notes_input.setPlainText(invoice['notes'])
            
            self.calculate_totals()
            
        except Exception as e:
            QMessageBox.critical(self, tr('common.error'), f"Error loading invoice: {str(e)}")
    ...
    def generate_invoice_number(self):
        """توليد رقم فاتورة"""
        return self.backend.generate_invoice_number("sales")
