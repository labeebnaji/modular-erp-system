...
class SalesOrderWidget(QWidget):
    ...
    def init_ui(self):
        ...
        # Sales Order form
        form_group_box = QGroupBox("Sales Order Details")
        ...
        # Icon placeholder: self.customer_id_input.addAction(QIcon("path/to/customer_icon.png"), QLineEdit.LeadingPosition)
        ...
        self.status_input.addItems(["Draft", "Confirmed", "Shipped", "Billed", "Cancelled"])
        form_layout.addRow(QLabel("Status:"), self.status_input)

        add_button = QPushButton("Add Sales Order")
        # Icon placeholder: add_button.setIcon(QIcon("path/to/add_icon.png"))
        add_button.clicked.connect(self.add_sales_order)
        ...
        # Sales Order table
        self.sales_orders_table = QTableWidget()
        self.sales_orders_table.setColumnCount(11) # Increased column count
        self.sales_orders_table.setHorizontalHeaderLabels(["ID", "Company", "Branch", "Customer ID", "Order No.", "Date", "Total Amount", "Total Tax", "Status", "Currency", "Created At"])
        self.sales_orders_table.horizontalHeader().setSectionResizeMode(QHeaderView.Stretch) # Stretch columns
        main_layout.addWidget(self.sales_orders_table)
    ...
    def load_sales_orders(self):
        self.sales_orders_table.setRowCount(0)
        sales_orders = self.sales_purchase_service.get_all_sales_orders()
        self.sales_orders_table.setRowCount(len(sales_orders))
        for row, order in enumerate(sales_orders):
            status_map = {0: "Draft", 1: "Confirmed", 2: "Shipped", 3: "Billed", 4: "Cancelled"}
            ...
            self.sales_orders_table.setItem(row, 0, QTableWidgetItem(str(order.id)))
            self.sales_orders_table.setItem(row, 1, QTableWidgetItem(company_name))
            self.sales_orders_table.setItem(row, 2, QTableWidgetItem(branch_name))
            self.sales_orders_table.setItem(row, 3, QTableWidgetItem(str(order.customer_id)))
            self.sales_orders_table.setItem(row, 4, QTableWidgetItem(order.order_no))
            self.sales_orders_table.setItem(row, 5, QTableWidgetItem(str(order.order_date)))
            self.sales_orders_table.setItem(row, 6, QTableWidgetItem(str(order.total_amount)))
            self.sales_orders_table.setItem(row, 7, QTableWidgetItem(str(order.total_tax)))
            self.sales_orders_table.setItem(row, 8, QTableWidgetItem(status_map.get(order.status, "Unknown")))
            self.sales_orders_table.setItem(row, 9, QTableWidgetItem(order.currency))
            self.sales_orders_table.setItem(row, 10, QTableWidgetItem(str(order.created_at.strftime('%Y-%m-%d %H:%M:%S'))))
    ...
    def add_sales_order(self):
        try:
            ...
            if not company_id or not branch_id or not customer_id or not order_no or not total_amount or not currency:
                QMessageBox.warning(self, "Input Error", "Company, Branch, Customer ID, Order No., Total Amount, and Currency are required.")
                return

            self.sales_purchase_service.create_sales_order(
            ...
            self.clear_form()
            self.load_sales_orders()
            QMessageBox.information(self, "Success", "Sales Order added successfully.")
